...
mod tests {
    ...
    fn test_ok() {
        ...
        assert_eq!(calc.eval("4 5 -").unwrap(),-1);
        assert_eq!(calc.eval("6 3 /").unwrap(),2);
        assert_eq!(calc.eval("9 4 %").unwrap(),1);
    }

    #[test]
    fn test_ng() {
        let calc=RpnCalculator::new(false);
        assert!(calc.eval("11*").is_err());
